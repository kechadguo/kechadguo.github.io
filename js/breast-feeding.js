...
window.BreastFeeding = {
  ...
  settings() {
    const defaults = { showDuration: true, showSide: true, quickTimeCount: 5, estimateMethod: 'off', estimateFactors: { duration: true, interval: true }, showEstimateIcon: true, chartDistinguish: true };
  ...
  estimate(record, baby, lastRecord) {
    ...
    if (s.estimateMethod === 'off') return { amount: null, method: 'off', baseAmount: null, intervalHours: Math.round(intervalHours * 10) / 10, durationFactor: null, intervalFactor: null, originalAmount: null, pumpRate: rate || null, monthAge };
  ...
  async openForm(existing = null) {
    ...
    App._showModal(existing ? '编辑亲喂记录' : '亲喂记录', `<div class="breast-form" id="breast-form">
      ...
      ${this.settings().estimateMethod !== 'off' ? `<div class="breast-estimate-box"><div><span>估算奶量：</span><strong id="breast-estimate-value">计算中...</strong><button type="button" class="icon-btn" title="估算说明" aria-label="估算说明" onclick="BreastFeeding.showEstimateInfo()">ⓘ</button></div><div class="breast-adjust"><button type="button" class="btn btn-outline btn-sm" onclick="BreastFeeding.adjust(-10)">-10</button><button type="button" class="btn btn-outline btn-sm" onclick="BreastFeeding.adjust(10)">+10</button><button type="button" id="breast-reset" class="btn btn-link btn-sm" style="display:none" onclick="BreastFeeding.resetAmount()">恢复原始估算</button></div></div>` : ''}
      <div class="form-group"><label>备注</label><input id="breast-note" class="form-input" value="${Utils.escapeHtml(existing?.note || '')}" placeholder="输入备注..."></div></div>
  ...
  _refreshEstimate() {
    ...
    if (settings.estimateMethod === 'off') { this._estimate = { amount: null, method: 'off' }; this._manualAmount = null; return; }
  ...
  async submit() {
    ...
    const estimate = this.estimate({ time: new Date(time).toISOString(), duration: this._duration }, Utils.getBabyInfo(), null);
    const estimateAmount = this._manualAmount || estimate.amount;
    const data = { type: 'breast_direct', feedingSubtype: 'breast_direct', time: new Date(time).toISOString(), duration: this._duration === 'none' ? null : this._duration, side: this._side === 'none' ? null : this._side, note: document.getElementById('breast-note')?.value || '', isEstimated: false, isManualAdjusted: false, inputMethod: 'table' };
    try {
      Utils.showLoading('保存中...');
      const directResult = this._editing?._id ? await API.updateFeeding(this._editing._id, data) : await API.createFeeding(data);
      if (!this._editing?._id && estimateAmount && this.settings().estimateMethod !== 'off' && directResult?.eventId && directResult?.recordId) {
        try {
  ...
  async savePumpOutput() {
    const outputMl = Number(document.getElementById('pump-output-amount')?.value);
    if (!(outputMl > 0)) return Utils.showToast('请输入有效泵奶量');
    const time = document.getElementById('pump-output-time')?.value || new Date().toISOString();
    const location = document.getElementById('pump-output-location')?.value || '';
    const expiresAt = document.getElementById('pump-output-expires')?.value || '';
    try {
      Utils.showLoading('入库中...');
      await API.createPumpOutput({ outputMl, time, storageLocation: location, expiresAt, note: document.getElementById('pump-output-note')?.value || '', inputMethod: 'table' });
      Utils.hideLoading(); App._closeModal(); Utils.showToast('泵奶已入库');
    } catch (error) { Utils.hideLoading(); Utils.showToast('入库失败：' + error.message); }
  },
  async loadInventory() { return window.API?.listInventoryBatches ? API.listInventoryBatches() : { records: [] }; },
  async loadInventoryTransactions() { return window.API?.listInventoryTransactions ? API.listInventoryTransactions() : { records: [] }; },
  async settleInventory(payload) { return API.settleBottle(payload); },
  async reverseInventory(transactionId, reason) { return API.reverseInventoryTransaction(transactionId, reason); },
  openPumpOutputForm() {
    App._showModal('泵奶入库', `<div class="form-group"><label>泵奶时间</label><input id="pump-output-time" type="datetime-local" class="form-input"></div><div class="form-group"><label>泵奶量（ml）*</label><input id="pump-output-amount" type="number" min="1" class="form-input"></div><div class="form-group"><label>储存位置</label><input id="pump-output-location" class="form-input" placeholder="冷藏/冷冻"></div><div class="form-group"><label>保质日期</label><input id="pump-output-expires" type="date" class="form-input"></div><div class="form-group"><label>备注</label><input id="pump-output-note" class="form-input"></div><button class="btn btn-primary btn-block" onclick="BreastFeeding.savePumpOutput()">确认入库</button>`);
  },
